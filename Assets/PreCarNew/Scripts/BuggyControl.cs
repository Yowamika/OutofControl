...
public class BuggyControl : MonoBehaviour
{
    ...
    // 速度
    public void SetSpeed(Vector3 velocity) { myRigidbody.velocity = velocity; }
    public Vector3 GetSpeed() { return myRigidbody.velocity; }
    ...
    void FixedUpdate()
    {
        ...
        : slip = Mathf.Lerp(slip, 0.01f, 0.02f);
            ...
            // 右クリックでフェード
            float restart = Input.GetAxis("Fire2");
